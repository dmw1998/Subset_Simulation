...
def mle_with_selective_refinement(N, p_0, L_b, y, L, gamma):
    ...
    if c_l <= y:
        failure_probability = p_0 * (np.sum(G <= y) / N) / denominator
        return failure_probability
    ...
    while c_l > y and l < L-1:
        ...
        if c_l <= y:
            failure_probability = (p_0 ** (l-1)) * (np.sum(G <= y) / N) / denominator
            return failure_probability
...
failure_probabilities = [mle_with_selective_refinement(N, p_0, 10, y, 5, gamma) for _ in range(1)]
print(failure_probabilities)
